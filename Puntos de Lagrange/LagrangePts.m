...
%% Integración de órbitas con ode45

tspan=[0,100];
InitCond=[L(in,1),L(in,2),L(in,3), 10^-2,0 ,0]';
opt=odeset('RelTol',1e-16,'AbsTol',1e-20);

tic
[~,U_sol]=ode45(@(t,U) systemFunction(t,U),tspan, InitCond);
toc
...
tic
[t,U_sol]=ode113(@(t,U) systemFunction(t,U),tspan, InitCond,opt);
toc 
